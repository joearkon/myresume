...
const ChatWidget: React.FC<ChatWidgetProps> = ({ language }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      {/* Chat Window */}
      {isOpen && (
        <div className="mb-4 w-80 md:w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-blue-800/50 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-fade-in-up origin-bottom-right transition-colors duration-300">
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-lg text-sm shadow-sm ${
                  msg.role === 'user' 
                    ? 'bg-blue-800 dark:bg-blue-600 text-white rounded-br-none' 
          ...
          <form onSubmit={handleSend} className="p-3 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex gap-2 transition-colors duration-300">
            <input
              ...
              className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-white text-sm rounded-md px-3 py-2 focus:outline-none focus:border-blue-800 dark:focus:border-blue-400 transition-colors"
            />
            <button 
              type="submit" 
              disabled={isLoading}
              className="bg-blue-800 dark:bg-blue-600 text-white p-2 rounded-md hover:bg-blue-700 dark:hover:bg-blue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <button 
        onClick={toggleChat} 
        className={`${isOpen ? 'bg-slate-600' : 'bg-blue-800 dark:bg-blue-600 hover:scale-110'} text-white p-4 rounded-full shadow-lg hover:shadow-blue-800/50 dark:hover:shadow-blue-400/50 transition-all duration-300 flex items-center justify-center group`}
      >
        {isOpen ? (
